...
void MQTTClient(Client* c, Network* network, unsigned int command_timeout_ms, unsigned char* buf, size_t buf_size, unsigned char* readbuf, size_t readbuf_size)
{
    ...
    c->inflight_qos = QOS0;   
    c->pubrel = 0;
    for (i = 0; i < MAX_INCOMING_QOS2_MESSAGES; ++i)
        c->incoming_qos2_messages[i] = 0;
}

int isQoS2msgidFree(Client *c, unsigned short id) {
    int i;
    for (i = 0; i < MAX_INCOMING_QOS2_MESSAGES;++i) {
        if (c->incoming_qos2_messages[i] == id)
            return 0;
    }
    return 1;
}
...
int useQoS2msgid(Client *c, unsigned short id) {
    int i;
    for (i = 0; i < MAX_INCOMING_QOS2_MESSAGES; ++i) {
        if (c->incoming_qos2_messages[i] == 0 ) {
            c->incoming_qos2_messages[i] = id;
            return 1; 
        }
    }
    return 0;
...
void freeQoS2msgid(Client *c, unsigned short id) {
    int i;
    for (i = 0; i < MAX_INCOMING_QOS2_MESSAGES; ++i) {
        if (c->incoming_qos2_messages[i] == id) {
            c->incoming_qos2_messages[i] = 0;
...
int cycle(Client* c, Timer* timer)
{
    ...
    switch (packet_type)
    {
        ...
        case PUBLISH:
        {
            ...
            else if (isQoS2msgidFree(c, msg.id)) {
                if (useQoS2msgid(c, msg.id))
                    deliverMessage(c, &topicName, &msg);
                else
                    fprintf(stderr, "Maximum number of incoming QOS2 messages exeeded!\n");
            }
