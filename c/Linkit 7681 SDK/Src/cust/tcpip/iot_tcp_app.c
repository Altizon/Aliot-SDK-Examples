...
int app_init_connection(void) {
	UIP_CONN *tcp_conn = NULL;
	uip_ipaddr_t raddr;
//	uint8 iot_srv_ip[MAC_IP_LEN] = { 138, 91, 36, 158 };
	uint8 iot_srv_ip[MAC_IP_LEN] = { 192, 168, 43, 58 };

	uip_ipaddr(raddr, iot_srv_ip[0], iot_srv_ip[1], iot_srv_ip[2],
			iot_srv_ip[3]);

	/* Specify remote address and port here. */
	tcp_conn = uip_connect(&raddr, HTONS(3000));

	if (tcp_conn) {
		tcp_conn->lport = HTONS(8888);

		//test if connection already open on 8888

		again: MyHttpPort = iot_get_ms_time() & 0xffff;
		int num = 0;
		UIP_CONN *poll;
		for (num = 0; num < UIP_CONNS; num++) {
			poll = &uip_conns[num];
			if (poll->lport == htons(MyHttpPort)) {
				goto again;
			}
		}
		printf_high("HTTP client port is %d ", MyHttpPort);
		tcp_conn->lport = htons(MyHttpPort);

		printf_high("connected successfully");
		return 1;
	} else {
...
//Handle for aborted, acked, closed states as well.

//	printf_high("Through TCP - %s ", uip_connected() ? "True" : "False");

	if (uip_acked()) {
		ConnectionAcked();
	}
	if (uip_aborted()) {
		printf_high("Connection Aborted\n");
		ConnectionClosed();
	}
	if (uip_closed()) {
		ConnectionClosed();
	}
	if (uip_newdata()) // there are data to be received and stored in uip_appdata
	{
		ResponseFromServer();
	}
	if (uip_poll()) {
...
void handle_tcp_app(void) {
	...
	if (uip_connected()) {
		...
		sprintf((char *) raddr, "%d.%d.%d.%d", uip_ipaddr1(uip_conn->ripaddr),
				uip_ipaddr2(uip_conn->ripaddr), uip_ipaddr3(uip_conn->ripaddr),
